...
export const QuzzPage = () => {
  ...
  const { questions, startQuizz, currentIndex,  chooseAnswer} = useQuesStore();
  ...
  return (
    <>
      <div className="bg-transparent  p-8 rounded-lg shadow-md text-center">
        ...
        <h1 className="text-white font-bold text-3xl">{currentQuestion.question}</h1>

        <div className="flex flex-col gap-2 mt-4 ">
          {currentQuestion.options.map((option, index) => {
            return <button key={index} onClick={ ()=> chooseAnswer(option)} className="p-4 border text-white border-gray-300 rounded-lg text-left hover:bg-[#808080]  transition duration-150 cursor-pointer">
              {option}
            </button>;
          })}
        </div> 
